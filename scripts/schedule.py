...
# or a PowerPoint deck <deck>-lecture.pptx with its exported .pdf and
# .notes.md (Row.lecture_pdf, Row.lecture_text);
...
class Row:
    ...
    def lecture(self) -> Path | None:
        """The week's deck: lectures-and-labs/<dir>/<deck>-lecture.pptx when the
        week has a PowerPoint deck, otherwise the Marp deck <deck>-lecture.md."""
        if not self.deck:
            return None
        pptx = self.path / f"{self.deck}-lecture.pptx"
        return pptx if pptx.is_file() else self.path / f"{self.deck}-lecture.md"
    ...
    @property
    def lecture_pdf(self) -> Path | None:
        """A PowerPoint deck's PDF, exported beside it by scripts/export_decks.py."""
        return self.lecture.with_suffix(".pdf") if self.is_pptx else None
    ...
    def lecture_text(self) -> Path | None:
        ...
        slide's text and speaker notes, generated by scripts/export_decks.py."""
        if not self.is_pptx:
            return self.lecture
...
def frontmatter(text: str) -> tuple[dict[str, str], str]:
    """A deck's `key: value` frontmatter as a dict, and the text after it."""
    text = text.replace("\r\n", "\n")
    m = re.match(r"---\n(.*?)\n---\n", text, re.S)
    if not m:
        return {}, text
...
def stale_exports(row: Row) -> list[str]:
    """What is wrong with a PowerPoint deck's exported PDF and text copy; empty
    when both are current, and for a Marp deck, which exports nothing.

    scripts/export_decks.py writes both beside the deck and records three
    SHA-256 hashes in the text copy's frontmatter: the deck's, the PDF's and
    the text's own. A deck saved again since, a PDF from another export, or a
    text copy edited by hand each break one of them."""
    if not row.is_pptx:
        return []
    missing = [p.name for p in (row.lecture_pdf, row.lecture_text) if not p.is_file()]
    if missing:
        return [f"{' and '.join(missing)} missing"]
    fields, body = frontmatter(row.lecture_text.read_text(encoding="utf-8"))
    problems = []
    if fields.get("deck-sha256") != sha256(row.lecture):
        problems.append(f"{row.lecture.name} has changed since it was exported")
    if fields.get("pdf-sha256") != sha256(row.lecture_pdf):
        problems.append(f"{row.lecture_pdf.name} is not the PDF exported with it")
    if fields.get("text-sha256") != hashlib.sha256(body.encode("utf-8")).hexdigest():
        problems.append(f"{row.lecture_text.name} was edited by hand (edit the deck instead)")
    return problems
